...
def remove_specified_content(text_output_path, words_to_remove):
    ...
    """
    pattern = '|'.join(sub for sub in words_to_remove)
    ...
    with open(text_output_path, 'w', encoding='UTF-8') as f:
        ...
        for line in content:
            ...
            if re.search(pattern, line):
                continue  # 如果包含指定词，则跳过该行
            
            # 写入到输出文件
            f.write(line)
    
    print(f"处理完成，已移除指定内容并保存至 {output_dir}")
